<script>
import AppLayout from '@/Layouts/AppLayout.vue'; // Используйте alias или относительный путь '../Layouts/AppLayout.vue'
...
export default {
    ...
    components: { Head },
    props: {
        product: { // Данные основного продукта/вариации
            type: Object,
            required: true,
        },
        availableExtras: { // Доступные допы (молоко, сиропы)
            type: Object,
            default: () => ({ milks: [], syrups: [] }),
        },
        errors: Object, // Ошибки валидации от сервера (если есть)
    },
    setup(props) {
        // --- Локальное состояние для выбора опций ---
        const selectedVariationId = ref(props.product.id); // ID выбранного размера/вариации
        const quantity = ref(1); // Выбранное количество
        const selectedOptions = ref({ // Выбранные доп. опции
            sugar_quantity: 0,
            has_cinnamon: false,
            milk_extra_id: null,
            syrup_extra_id: null,
        ...
        const isLoading = ref(false); // Флаг загрузки для кнопки "Добавить"

        // --- Вычисляемые свойства ---

        // Находим детали выбранной вариации (для актуальной цены)
        const currentVariation = computed(() => {
            return props.product.variations?.find(v => v.id === selectedVariationId.value)
                || { id: props.product.id, price: props.product.min_price }; // Фоллбэк
        });

        // Вычисляем цену за 1 шт с учетом допов
        const pricePerItemWithOptions = computed(() => {
            ...
            // Добавьте расчет цены за платные сахар, корицу, сгущенку, если нужно

            return basePrice + extrasPrice;
        });

        // Общая сумма для выбранного количества и опций
        ...
        const addToCart = () => {
            ...
            router.post(route('cart.add'), {
                product_id: selectedVariationId.value, // ID выбранного размера
                quantity: quantity.value,              // Выбранное количество
                options: selectedOptions.value,          // Объект с выбранными опциями
            }, {
                preserveScroll: true,
                onSuccess: () => {
                    console.log('Product added with options!');
                    alert(`${props.product.name} добавлен в корзину!`);
                },
                onError: (errors) => {
                    console.error('Failed to add product with options:', errors);
                    alert('Ошибка добавления в корзину: ' + JSON.stringify(errors));
                },
                onFinish: () => {
        ...
        return {
            ...
            changeSugar,
            addToCart,
            canShowMilkSelector, // Передаем как computed
            canShowSyrupSelector, // Передаем как computed
            product: props.product, // Для прямого доступа к product в шаблоне
            availableExtras: props.availableExtras, // Для прямого доступа к extras
        };
    }
}
</script>
...
<template>
    ...
    <div class="py-12 bg-white"> <!-- Изменен фон на белый -->
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">

                <!-- Левая колонка: Изображение -->
                <div class="px-4 sm:px-0 flex justify-center items-start"> <!-- Добавлен flex для центрирования -->
                    <div class="w-full max-w-md aspect-[4/3] bg-gray-100 rounded-lg overflow-hidden shadow-md"> <!-- Ограничиваем размер и соотношение -->
                ...
                <div class="bg-gray-50 rounded-lg shadow-md p-6 md:p-8 flex flex-col">
                    ...
                    <p class="text-2xl font-semibold text-gray-800 mb-4">
                        ...
                        <span v-if="product.variations && product.variations.length <= 1" class="text-base font-normal text-gray-500">/ шт</span>
                    </p>
                    ...
                    <div v-if="product.variations && product.variations.length > 1" class="mb-4">
                        ...
                        <div class="flex flex-wrap gap-2">
                            <button v-for="variation in product.variations" :key="variation.id"
                                ...
                                <!-- Показываем цену для размера, если она отличается от базовой -->
                                <!-- <span v-if="variation.price !== currentVariation.price">({{ formatPrice(variation.price) }} ₽)</span> -->
                            </button>
                        </div>
                    </div>

                    <!-- Выбор Опций (показывается только для Кофе и Сырников) -->
                    <div v-if="product.category_id === 3 || product.can_add_condensed_milk" class="space-y-3 mb-6 text-sm border-t border-gray-200 pt-4">
                        <h4 class="text-sm font-medium text-gray-800 mb-2">Дополнительно:</h4>

                        <!-- Молоко -->
                        <div v-if="canShowMilkSelector && availableExtras.milks.length > 0" class="flex items-center gap-2">
                            <label :for="`prod-milk`" class="text-gray-600 w-16 shrink-0">Молоко:</label>
                            <select id="prod-milk"
                                    :value="selectedOptions.milk_extra_id ?? ''"
                        ...
                        <!-- Сироп -->
                        <div v-if="canShowSyrupSelector && availableExtras.syrups.length > 0" class="flex items-center gap-2">
                            <label :for="`prod-syrup`" class="text-gray-600 w-16 shrink-0">Сироп:</label>
                            <select id="prod-syrup"
                                    :value="selectedOptions.syrup_extra_id ?? ''"
                        ...
                        <div v-if="product.can_add_sugar" class="flex items-center gap-2">
                            ...
                            <div class="flex items-center border border-gray-300 rounded-full overflow-hidden">
                                <button @click="changeSugar(-1)" :disabled="selectedOptions.sugar_quantity <= 0" class="px-3 py-1 text-gray-600 hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed">-</button>
                                <span class="px-3 py-1 text-center font-medium text-gray-700 w-10 text-sm">{{ selectedOptions.sugar_quantity || 0 }}</span>
                                <button @click="changeSugar(1)" :disabled="selectedOptions.sugar_quantity >= 3" class="px-3 py-1 text-gray-600 hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed">+</button>
                            </div>
                        </div>
                        ...
                        <div v-if="product.can_add_cinnamon" class="flex items-center gap-2">
                            <label class="flex items-center text-gray-600 cursor-pointer">
                                <input type="checkbox"
                                       :checked="selectedOptions.has_cinnamon"
                        ...
                        <div v-if="product.can_add_condensed_milk" class="flex items-center gap-2">
                            <label class="flex items-center text-gray-600 cursor-pointer">
                                <input type="checkbox"
                                       :checked="selectedOptions.has_condensed_milk"
                                       @change="toggleOption('has_condensed_milk')"
                                       class="rounded border-gray-300 text-emerald-600 shadow-sm focus:ring-emerald-500 h-4 w-4">
                                <span class="ml-2">Сгущенка</span>
                            </label>
                        </div>
                    </div> <!-- Конец блока с опциями -->
                    ...
                    <div class="flex flex-col sm:flex-row items-center gap-4 mt-auto pt-6 border-t border-gray-200">
                        ...
                        <div class="flex items-center border border-gray-300 rounded-full overflow-hidden flex-shrink-0">
                            <button @click="changeQuantity(-1)" :disabled="quantity <= 1" class="px-4 py-2 text-gray-600 hover:bg-gray-100 focus:outline-none disabled:opacity-50 text-lg">-</button>
                            <span class="px-5 py-2 text-center font-medium text-gray-800 w-16 text-lg">{{ quantity }}</span>
                            <button @click="changeQuantity(1)" :disabled="quantity >= 10" class="px-4 py-2 text-gray-600 hover:bg-gray-100 focus:outline-none disabled:opacity-50 text-lg">+</button>
                        </div>
                        ...
                        <button @click="addToCart"
                                ...
                                :class="[
                                     ...
                                     isLoading ? 'bg-emerald-300 cursor-not-allowed' : 'bg-emerald-500 hover:bg-emerald-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500'
                                 ]">
...
<style scoped>
/* Стили для ProductShow, если нужно */
</style>

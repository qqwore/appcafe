...
<template>
    ...
    <h1 class="text-2xl font-semibold text-gray-700 mb-6">Меню на день (Сток)</h1>

    <!-- Форма добавления стока -->
    <form @submit.prevent="submitAddStock" class="bg-white p-6 rounded-lg shadow mb-8">
        <h2 class="text-lg font-medium text-gray-800 mb-4">Добавить приход товара</h2>
        <div class="flex flex-col sm:flex-row items-end gap-4">
            ...
            <div class="flex-grow w-full sm:w-auto">
                ...
                <select id="product_id" v-model="addStockForm.product_id" required
                        class="w-full border-gray-300 rounded-md shadow-sm focus:border-emerald-300 focus:ring focus:ring-emerald-200 focus:ring-opacity-50">
                ...
                <div v-if="addStockForm.errors.product_id" class="text-red-500 text-xs mt-1">
                    {{ addStockForm.errors.product_id }}
                </div>
            </div>
            <!-- Количество -->
            <div class="w-full sm:w-32 flex-shrink-0">
                <label for="quantity_to_add" class="block text-sm font-medium text-gray-700 mb-1">Количество, шт</label>
                <input type="number" id="quantity_to_add" v-model="addStockForm.quantity_to_add" required min="1"
                       max="999"
                       class="w-full border-gray-300 rounded-md shadow-sm focus:border-emerald-300 focus:ring focus:ring-emerald-200 focus:ring-opacity-50">
                <div v-if="addStockForm.errors.quantity_to_add" class="text-red-500 text-xs mt-1">
                    {{ addStockForm.errors.quantity_to_add }}
                </div>
            </div>
            <!-- Кнопка -->
            <div class="flex-shrink-0">
                <button type="submit"
                        :disabled="isAddingStock || !addStockForm.product_id || !addStockForm.quantity_to_add"
                        class="px-5 py-2 bg-emerald-500 hover:bg-emerald-600 text-white font-semibold rounded-md shadow-sm disabled:opacity-50 disabled:cursor-not-allowed">
                    {{ isAddingStock ? 'Добавляем...' : 'Добавить' }}
                </button>
            </div>
        </div>
        <!-- Общая ошибка формы, если есть -->
        <div v-if="addStockForm.hasErrors && !addStockForm.errors.product_id && !addStockForm.errors.quantity_to_add"
             class="text-red-500 text-sm mt-2">
            Не удалось добавить приход. Проверьте данные.
        </div>
    </form>
    ...
    <div class="bg-white shadow rounded-lg overflow-hidden">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tr v-if="stockableProducts.length === 0">
                <td colspan="3" class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">Нет товаров для
                    отслеживания стока.
                </td>
            </tr>
            <tr v-for="product in stockableProducts" :key="product.id">
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ product.name }}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ product.count ?? 0 }}</td>
                <!-- <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <button @click="removeStockItem(product.id)" class="text-red-600 hover:text-red-900">Удалить</button>
                </td> -->
            </tr>
            </tbody>
        </table>

            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between items-center h-16">
                    ...
                    <div class="hidden md:flex items-center space-x-4">
                        ...
                        <div v-if="$page.props.cart && $page.props.cart.count > 0" class="flex items-center space-x-1 text-sm">
                            <!-- Счетчик -->
                            <span class="inline-flex items-center justify-center px-2 py-0.5 text-xs font-bold leading-none text-emerald-100 bg-emerald-600 rounded-full">
                    ...
                    <div class="md:hidden flex items-center">
                        ...
                        <a v-if="$page.props.auth.user"
                           ...
                           title="Корзина">
                            ...
                            <span v-if="$page.props.cart && $page.props.cart.count > 0" class="absolute -top-2 -right-2 inline-flex items-center justify-center px-1.5 py-0.5 text-xs font-bold leading-none text-emerald-100 bg-emerald-600 rounded-full">
                                 {{ $page.props.cart.count }}
                             </span>
                        </a>
                        <!-- Кнопка Гамбургер -->
            ...
            <div v-show="mobileMenuOpen" class="md:hidden border-t border-gray-200">
                ...
                <div class="pt-4 pb-3 border-t border-gray-200">
                    ...
                    <div v-if="$page.props.auth.user" class="px-5 mb-3">
                        <a :href="routeExists('cart.index') ? route('cart.index') : '/cart'" class="flex justify-between items-center rounded-md text-base font-medium text-gray-600 hover:bg-gray-50 hover:text-gray-800 px-3 py-2">
                            <span>Корзина</span>
                            <span v-if="$page.props.cart && $page.props.cart.count > 0" class="ml-2 inline-flex items-center justify-center px-2 py-0.5 text-xs font-bold leading-none text-emerald-100 bg-emerald-600 rounded-full">
                                 {{ $page.props.cart.count }} товар(ов) на {{ formatPrice($page.props.cart.total_default_price) }} ₽
                             </span>
                        </a>
                    </div>
        ...
        <footer class="bg-stone-800 text-stone-400 py-8 mt-auto">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 md:grid-cols-3 gap-8 items-center text-center md:text-left">
                ...
                <div class="text-sm flex items-center justify-center text-stone-300">
                    <svg class="w-5 h-5 mr-2 inline-block flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clip-rule="evenodd"></path></svg>
